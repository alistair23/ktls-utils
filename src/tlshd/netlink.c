...
static int tlshd_genl_valid_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb[HANDSHAKE_A_ACCEPT_SOCKFD]) {
		...
		tlshd_log_debug("parms->peeraddr_len: %d\n", parms->peeraddr_len);

		int ret = getpeername(parms->sockfd, parms->peeraddr, &parms->peeraddr_len);
		tlshd_log_debug("getpeername(): %d\n", ret);
		if (ret == -1) {
...
void tlshd_genl_done(struct tlshd_handshake_parms *parms)
{
	...
	tlshd_log_debug("parms->num_remote_peerids: %d", parms->num_remote_peerids);
	tlshd_log_debug("parms->remote_peerid[0]: 0x%x", parms->remote_peerid[0]);
	...
	err = nla_put_s32(msg, HANDSHAKE_A_DONE_KEY_SERIAL,
			  parms->key_serial);
	if (err < 0) {
