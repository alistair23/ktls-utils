...
/**
 * tlshd_keyring_put_session - Store a gnutls session
...
key_serial_t tlshd_keyring_put_session(gnutls_session_t session)
{
	...
	if (serial < 0) {
		tlshd_log_perror("Failed to save gnutls session.");
		return -1;
	}
...
bool tlshd_keyring_get_session(key_serial_t serial, gnutls_session_t session)
{
	...
	/* Clear the session key to avoid filling up the keyring */
	ret = keyctl_invalidate(serial);
	if (ret < 0) {
		tlshd_log_perror("Failed to invalidate the old gnutls session.");
	}
